...
pub fn set(t: *TBTable, comptime dtz: bool, buf: []const u8) bool {
    ...
    // Check the byte after the magic, which carries Split(1)/HasPawns(2), against the registry.
    // Both bits restate what the FILENAME already told it, and every group width below is
    // derived from the registry's answer -- so a header that disagrees is not the table this
    // call is about to index, and those widths would be read against the wrong geometry.
    // Upstream asserts both, which a release build compiles out; refuse instead.
