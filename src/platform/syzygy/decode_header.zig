...
pub fn setSizes(gpa: std.mem.Allocator, d: *PairsData, buf: []const u8, pos: *usize) !void {
    ...
    @memset(d.symlen, 0);
    // Colour with three states, not two. GREY marks a symbol still on the DFS path, which is
    // what makes a cycle detectable at all: with two states a symbol is marked before its
    // subtree is walked, so a cycle reads as "already computed" and the table is accepted with
    // symlen[] entries left at zero.
