...
test "classify: non-decisive scores are kind 0" {
    ...
    try std.testing.expectEqual(@as(i32, 0), r.kind);
    try std.testing.expectEqual(@as(i32, 0), r.plies);
    try std.testing.expectEqual(@as(i32, 0), r.win);
}
...
test "classify: tablebase-range scores are kind 1, distance-to-value_tb encoded" {
    ...
    try std.testing.expectEqual(@as(i32, 1), r.kind);
    try std.testing.expectEqual(@as(i32, 1), r.win);
...
test "classify: mate scores are kind 2, sign-encoded plies" {
    ...
    try std.testing.expectEqual(@as(i32, 2), win.kind);
    try std.testing.expectEqual(@as(i32, 32000 - 31950), win.plies);
    try std.testing.expectEqual(@as(i32, 1), win.win);

    const loss = classify(-31950, 31507, 31753, 32000);
    try std.testing.expectEqual(@as(i32, 2), loss.kind);
    try std.testing.expectEqual(@as(i32, -(32000 - 31950)), loss.plies);
    try std.testing.expectEqual(@as(i32, 0), loss.win);
}
