...
// Mirror the search Stack in memory (src/search.h). Only the scalar fields used
// by the search helpers are read; the layout/size must match for ss-N stack
// arithmetic.
// Re-export the search POD types (SearchStack/CorrectionBundle/PVMoves/RootMove) from the
...
// Treat the Worker's WorkerHistories sub-block as the typed WorkerLayout.histories field now,
