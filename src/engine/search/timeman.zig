...
pub fn init(input: TimemanInput) TimemanOutput {
    ...
    // Convert `movetime` into the unit the search compares it in. Under `nodestime`
    // checkTime's `elapsed` is a NODE count, so an unconverted figure stops the search after
    // N nodes rather than after N milliseconds' worth of them. Convert BEFORE the no-clock
    // early return, so `go movetime N` carrying no clock is converted too.
