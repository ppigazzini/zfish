...
pub fn checkTime(ctx: *const QCtx) void {
    ...
    //
    // The conversion belongs there, beside the clock and the increment it converts alongside;
    // do not re-scale here. ../rfish surfaced the un-converted version by giving the two
    // quantities different types, which refused the comparison until it was written down.
