...
pub fn syzygyExtendPv(
    ...
    // Step 2: extend toward mate by always taking the top-ranked move. Bounded by the walk's
    // own terminations -- mate, a draw, or the deadline -- and by nothing else: the list grows,
    // as upstream's root PV does (ee515ad9), so a mate line longer than MAX_PLY is reported
    // whole rather than cut off at the buffer that used to hold it.
