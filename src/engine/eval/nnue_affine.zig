...
// dots, selected at comptime by the target features. Bench-verified bit-exact (2884956 on every
...
// as the SSSE3 path (a pair sums inside i16), so it is bit-identical -- signature 2884956 holds.
...
// integer dot, so the reduction order is irrelevant and it stays bit-exact (signature 2884956);
