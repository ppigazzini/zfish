...
pub fn transformBucket(
    ...
    // upstream 0c8c71f3 draws the same line.
...
// the non-zero chunk mask for it -- upstream 0c8c71f3 (transform_perspective). The caller
// resolves perspectives[p] and hands over that half, so nothing here needs to know which side is
// to move: `perspective` is a position in the OUTPUT, the index space the offset is derived
// from, and never a colour.
//
