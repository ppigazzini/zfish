...
pub fn fullAppendActive(
    ...
    // colour loop below is absolute (WHITE, BLACK) rather than perspective-relative: the
    // accumulator SUMS the rows this list names, so the order it names them in is not an
    // observable.
