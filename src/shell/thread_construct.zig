// Verify the construction of the ThreadPool / Thread graph.
//
// Note the ThreadPool reproduces a pinned observable layout (stop@0,
...
//
// Assert the freshly constructed pool against the pinned
// offsets. Follow the same model as the worker constructor. Stay read-only; panic on drift.
...
pub fn verifyThreadGraph(pool: *const worker_layout.ThreadPool, requested: usize, bound: usize) void {
    ...
    // Expect the leading atomic pair zeroed right after construction (no search has started).
    ...
    // Treat threads as a Zig slice of Thread* addresses; size == requested.
    ...
    // Treat boundThreadToNumaNode as a []usize slice of NumaIndex; size == bound.
