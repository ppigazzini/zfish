...
// bench signature 2516158, so bench itself is this data's regression gate.
// Drive the richer `benchmark` command with `BenchmarkPositions`. Keep pure data, std-only.

const std = @import("std");

pub const Defaults = [_][]const u8{
...
pub const BenchmarkPositions = [_][]const []const u8{
    &.{
...
// ---- tests (data integrity; bench 2516158 is the behavioural gate for Defaults) ----

const testing = std.testing;

test "bench position tables are well-formed and non-empty" {
    try testing.expect(Defaults.len > 0);
    for (Defaults) |line| try testing.expect(line.len > 0);

    try testing.expect(BenchmarkPositions.len > 0);
    for (BenchmarkPositions) |game| {
        try testing.expect(game.len > 0);
        for (game) |fen| try testing.expect(fen.len > 0);
    }
}
