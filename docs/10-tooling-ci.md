...
| `type-refusal` | Each illegal form [09-type-design](09-type-design.md) claims is unrepresentable fails to compile, with the quoted message, and each legal form still compiles. |
...
| Do the types refuse what the docs claim? | `type-refusal` | Whether any call site uses the type, and whether some *other* transposition is still expressible. |
| Is the module graph still a DAG, and is every file compiled? | `arch-report` | Whether an edge that exists is one that *should*. It reports coupling and gates only the binary properties. |
| Do the docs still match the tree? | `docs-lint` | Whether a sentence is TRUE. It settles links, paths, symbols and the anchor — the cheap half, so review can spend attention on the expensive half. |
| Can each gate still FAIL? | `negative_control.sh` | A gate nobody wrote. It mutates the tree per row and requires red; it cannot invent the row. |
| Is every gate actually dispatched? | `lane-coverage` | Whether the gate it dispatches checks anything. |
| …and the tools no lane invokes? | `tools_smoke.sh` | The same. It proves they still run, not that they are right. |
| Is every golden read by something? | `golden-coverage` | Whether the golden's contents are correct. |
| Does hostile input stay inside the arrays? | `parity-malformed` | Input nobody has described yet — that is the fuzzers' budget, not this one's. |
| Do the Syzygy probes match the oracle? | `tb-init`/`tb-wdl`/`tb-dtz`/`tb-root`/`tb-search` | **The decoder.** Measured by mutation: a `decompress_pairs` defect moving 130 of 1422 probe values leaves all five green. `tb-cursed` is the only carrier, and it is local-only ([05-tablebases](05-tablebases.md)). |
| Does every ISA tier agree? | `arch_determinism.sh` | Any tier this host cannot execute — it names those rather than skipping them silently. |
...
| `parity-malformed` | The one gate whose input is **hostile** rather than merely unusual. Every other gate feeds the engine a legal command sequence and asks what it does; this copies a real 3-man table, mutates named header fields and payload bytes, and asks whether the parser stays inside its arrays. Each of the ten fixtures must be *survived* — clean exit, a legal bestmove, no hang — rather than diagnosed, because the Syzygy format records nothing that says which value was meant, so a reader cannot tell a mutated `lowestSym` from an intended one and must not pretend to. `signature` is green with every bound in `decode_header.zig` reverted, since the bench reads no file the engine did not ship with, and the fuzz targets hunt *undescribed* input on a nightly budget — so before this gate nothing asked whether yesterday's refusal still held. Needs the 3-man corpus (fixtures are mutations of real tables, not crafted blobs) and SKIPs loudly without it; `parity` only, like the other tablebase gates. |
...
| `type-refusal` | The types hold. Every refusal [09-type-design](09-type-design.md) states in prose is written as a snippet beside its module and compiled: the wrong form must fail **with the message the page quotes**, and the right form must still build. Both halves are load-bearing. A row that merely checks for a non-zero exit scores a rig fault as a detection — the first draft of one row failed with "parameter of type … not allowed in function with calling convention 'x86_64_sysv'", which is not a type refusal at all; and a second compiled cleanly when it should have failed, because `comptime { _ = f; }` references a function without analysing its body. `qsearchImpl`'s `@compileError` on `.root` is deliberately NOT a row: reaching it means resolving the whole module graph, so the snippet would fail with "no module named 'worker_layout'" — a rig fault wearing a refusal's clothes. |
...
variant: no value moves, so the anchor, every golden and the whole search read identically. The
`test` row's mutant stops giving large-page arenas back: `parity-valgrind` and `parity-teardown`
stay green over a build that releases no arena at all, because memcheck reports a definite leak
only for allocations it INTERCEPTS and the Linux large-page path is now an anonymous `mmap`
([06-platform.md](06-platform.md)). `memory.liveLargePageBlocks()` is the instrument that can
see it, and this row is what keeps proving it can — a sibling port made the same mutation a
permanent row rather than a thing done once, which is the difference between a gate and a
...
| `zig build authors-lint` | Holds `AUTHORS` to being upstream's own file, byte for byte, at the pinned commit. It is carried verbatim because zfish reproduces upstream's search and evaluation and is a derived work, so the attribution travels with it ([11-references.md](11-references.md)) — and "verbatim" is a claim, which is exactly the kind that rots when nothing settles it. It had: by the `2edd935b` resync the file was **fifteen names short**, accumulated over several syncs, including the one upstream `22dfb404` added inside the window being ported — because the commit that ported the rest of that commit read the source files and not this one. A sibling port found the same omission by diff, which is the whole design here. Diffs against `UPSTREAM_BASE`, not a fetched branch: a moving reference would go red on every upstream push rather than on a real omission. A missing upstream object is a **refusal**, never a skip — same lane and same reason as `upstream-map` above. |
