...
| `zig build upstream-map` | The gate form (LOCAL-ONLY — needs the upstream git objects a CI checkout lacks): declared-map ROT fails; the uncovered-surface count is RATCHETED (baseline in build.zig; lower it as citations land, never raise it). |
