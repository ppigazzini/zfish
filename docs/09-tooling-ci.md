...
| `zfish_fuzz.yml` | nightly schedule, dispatch | Two jobs. **Board targets**: `zig build fuzz --fuzz` under `ReleaseSafe` for a bounded budget, mutating toward new coverage over FEN-parse → `generateLegal` → make/unmake; a SIGINT at the budget with no crash passes, a target crash fails early with the safety-check trace and the reproducer. **UCI command loop**: `tools/uci_fuzz.py` against a `ReleaseSafe` build for ~10 min, seed defaulting to the wall clock so successive nights walk fresh input space; the seed prints first, so a red run names its one-flag reproducer. |
