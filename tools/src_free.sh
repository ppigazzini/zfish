...
if [ "$cpp_sf" -eq 0 ] && [ "$cpp_std" -eq 0 ] && [ "$sig" = "2067208" ]; then
    echo "src-free: OK — src-free (no C++ Stockfish/libc++ symbols in the shipped binary; bench 2067208)"
    exit 0
fi

echo "src-free: REGRESSION — src-free invariant violated (want cpp_sf=0 cpp_std=0 bench=2067208)." >&2
exit 1
