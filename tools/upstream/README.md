...
| `01d71fc9` Fix UB when passing plain char to `<cctype>` | four call sites cast `char` to `unsigned char` before `isspace`/`islower`/`tolower`, because a negative `char` is UB there. Every counterpart here already takes a `u8` -- `uci_strings.isSpaceByte(byte: u8)`, and the flip transform walks a `[]u8` -- so there is no sign to extend. The same shape as the `d68041e3` row above: type the ACCESSOR and the defect stops being representable ([09-type-design.md](../../docs/09-type-design.md)). |
| `06675f70` update pairwise mul explanation | deletes the stale narrative in `transform` that said the pairwise product is divided by 128, then walked through scaling the accumulator weights by 2 to reach 512. `nnue_accumulator.transformPerspective` states the identity the code actually implements -- `(c0*c1) >> 9 == ((c0<<7) * c1) >> 16`, the product over 512 -- so there was never a 128 story here to correct. |
| `1dc0912d` Improve comments | rewraps upstream's own `tt.cpp` / `tt.h` / `misc.cpp` prose and gives one-line headers to functions that had none. zfish's comments are written here, in imperative mood ([12-writing.md](../../docs/12-writing.md)), and every substantive claim it carries is already at the same site: the probe read order is a `comptime` `@offsetOf` block in `tt_types.zig`, and the racy-underflow guard is `depthSaturatingSub`'s whole reason. Its ONE claim zfish did not state -- that generations are clock arithmetic, so `0 - 1` must be 31 and the subtract wraps -- is now on `entryRelativeAge`. |
| `342c24b4` Fix AVX2 Compiler Pipeline, `59aae690` Add -ftrapv to CI, `4c991722` Increase go-mate positions to 1000 | three of upstream's own GitHub Actions workflows (`avx2_compilers.yml`, `sanitizers.yml`, `matetrack.yml`). zfish's lanes are its own; the overflow trapping `-ftrapv` buys is what `zig build test -Doptimize=ReleaseSafe` already gives every unit test here. |
| `86433ca7` Add iterator header to numa.h | a missing `#include <iterator>`. Zig has no include graph to repair. |
| `b6928867` Fail reproducibility checks on engine errors | hardens `tests/reprosearch.sh`: `set -o pipefail` plus a `grep '^info '` so an engine ERROR line carrying a number stops being counted as node output and a dead engine stops exiting 0 through the pipe. zfish's reproducibility check is the Zig parity harness (`parity-mt`, `search-parity`), which compares whole typed records against a golden -- an error line or a missing node count is a MISMATCH by construction, not a pipeline stage that can swallow it. Its AUTHORS half IS carried, below. |
...
| `b180d657` | the version string goes back to `dev` after the 19 release (`shell/misc.zig`), so `uci` reports `dev-<date>-<sha>` again. |
| `0e24f3f2` | upstream dropped `ISearchManager`/`NullSearchManager` for a null `unique_ptr` on non-main workers. `WorkerLayout.manager` was already `?*SearchManager` and every reader is behind `thread_idx == 0`, but `workerBuild` still minted a zeroed manager per helper -- the Null Object under another name. `makeSearchManager` now serves the main thread only and returns an error union, so OOM stays distinguishable from "no manager wanted". |
| `5ca4bfe7` | `timeman.init` scales `opt_scale` by `1 + 0.9 * min(time_advantage, 0)`, so a clock deficit cuts the optimum budget. `TimemanInput` gains `opp_time_ms` -- the first formula that reads both clocks. Both escapes are load-bearing: `nodestime` (no deterministic opponent node budget) and `movestogo == 1` (an opponent who has already moved carries the next cycle's increment). |
| `9287f165` | cyclic time controls under `nodestime`: `TimeManagement` gains `cyclic_budget` (the first limit MINUS the increment already inside it) and `previous_movestogo`, and one cycle is added to `available_nodes` when `movestogo` climbs back instead of counting down. `clearTimeman` resets the latter. |
| `55cf867c` | `dbgHitOn` returns its condition and `dbgMeanOf`/`dbgStdevOf`/`dbgExtremesOf` return their value, so a branch under inspection is not evaluated twice. Zig's must-use rule makes the return load-bearing: the two in-file tests stopped compiling and now assert the pass-through. `dbgCorrelOf` takes two values and stays void, as upstream left it. |
| `5e723b39` | `board_core`'s runtime pawn push-or-attacks table goes: the NNUE threat update it was built for reads `nnue_feature_bb`'s own comptime version. |
...
| `4d65257b` | `timeman.init` takes the sub-second move-horizon reduction only when `movestogo` is 0. In a cyclic time control the token is the real count of moves before the clock is topped up, and reducing below it budgets each move as though the session ended at the next control -- upstream measured 557 losses on time at 40/1 against zero after the fix. Held by a unit test, because no gate here runs the clock. |
| `3f6f417b` | the WDL polynomial's `as`/`bs` coefficients (`uci_wdl.zig`), refit on 2M games since `229f6339`. `a` is the centipawn normaliser, so every `score cp` and `wdl` triple this engine prints moves; not one node does. Eight goldens rehardened. |
| `47be34c5` | `conthist_weights` halve to `{520, 390, 145, 251, 66, 209}` and the divisor with them (`bonus * weight * multiplier / 65536`, `search_stats.zig`). Exact halves over a halved denominator, so the quotient is unchanged wherever the old 3-way product FIT an `int` -- and different wherever it did not, which is upstream's whole point: the bench (depth 13) is untouched at 2497913 while `bench-matrix`'s node-limited 200000-node row moves 9806827 -> 9807129. |
| `6e4c77f5` | `timeman.init` floors the scaled clock at 1. Under `nodestime` a budget smaller than `npmsec` divides to zero, `log10(0)` is `-inf`, and the `+inf` `opt_scale` that follows reaches `@intFromFloat` -- which is UB in C++ and an ABORT in ReleaseSafe here. Held by a unit test; the mutation reproduces the abort. |
| `edb0d9db` | the version string becomes `19` (`misc.zig`), so the banner is `Stockfish 19` rather than `Stockfish dev-DATE-SHA`, and AUTHORS takes the one name `01d71fc9` added. |
