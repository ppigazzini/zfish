...
#
# Usage: h9_src_free.sh <stockfish-bin>   (run with cwd = net/ so bench finds the net)
set -u
...
if [ "$cpp_sf" -eq 0 ] && [ "$cpp_std" -eq 0 ] && [ "$sig" = "2067208" ]; then
    echo "h9: OK — src-free (no C++ Stockfish/libc++ symbols in the shipped binary; bench 2067208)"
    exit 0
fi

echo "h9: REGRESSION — src-free invariant violated (want cpp_sf=0 cpp_std=0 bench=2067208)." >&2
exit 1
