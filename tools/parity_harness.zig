...
//     run bench and assert `Nodes searched` == expected (the 2718396 arch/OS invariant).
...
// (16/1/depth-13 -> 2718396) never exercises. Verify equal to the pristine upstream oracle
...
const bench_matrix_configs = [_][]const u8{
    ...
    "128 1 13", // hash size (2561648 != the default 2718396 -> the TT-sizing path)
...
fn buildTbDtz(gpa: std.mem.Allocator, io: Io, bin: []const u8) ![]u8 {
...
// signature: assert bench reports the exact node count (the 2718396 arch/OS invariant).
